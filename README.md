...
> dropdown now stays centered and doesnt move when scrolling
...
> clicking the button now changes the button's state.
...
> page no longer  crashed when selecting all employees.
...
> clicking view more now appends data to old data.
...
> moved isloading(false) to before you fetch the other data, but ideally i would create new state to store the loading for the employee and actual data loading separate
...
>> added check for number of job postings loaded, also checks if selected employee before showing view more
...
> checked values now persist and clear cache when changing values
